...
- Сверх этого `main.js` на загрузке спрашивает релиз ещё раз и обновляет версию, дату и адреса — на случай релиза между деплоями. Систему определяет браузер (`userAgentData`, иначе `userAgent`): главная кнопка берёт адрес у кнопки своей системы и называется «Скачать для …», а её дубль в нижнем ряду скрывается. Если запрос не прошёл (нет сети, исчерпан лимит 60 запросов в час на IP), остаются значения из разметки. Это единственный внешний запрос со страницы.
