...
- Версия и дата сборки не вбиты руками: `main.js` берёт их из последнего релиза приложения через GitHub API и подставляет в шапку, FAQ, блок «Скачать» и JSON-LD. В разметке лежит запасное значение — если запрос не прошёл (нет сети, исчерпан лимит неавторизованных запросов к API), страница остаётся с ним. Это единственный внешний запрос со страницы.
