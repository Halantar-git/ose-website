...
.github/check-site.py          сами проверки: ссылки, размеры, места подстановки
.github/stamp-release.py       версия и ссылки из релиза в разметку, на деплое
```
...
`og:image` соответствует своему файлу, места подстановки релиза (`data-release-version`,
`data-release-date`, `data-download-os`) на месте, а `stamp-release.py` на поддельном релизе
подставляет версию, дату и все три ссылки. Тот же скрипт стоит в `pages.yml` перед выкладкой:
битая разметка не доедет до сайта.
...
3. **Скриншоты.** Вставлены из приложения: в hero — `assets/img/main.webp`, в галерее `#screens` — остальные 9. Вертикальные скриншоты пульта помечены `shot--tall`, чтобы не растягиваться на всю колонку.
   Alt-тексты написаны по именам файлов — вычитайте их в `index.html` и поправьте под фактические экраны. Особенно `ed-3`, `poll2`, `remote2` и `term`.
   Клик по скриншоту открывает его крупно: лайтбокс собран в `assets/js/main.js` без библиотек.
4. **Политика конфиденциальности** (`privacy.html`) описывает сайт и приложение. Если в приложении появится новая интеграция или сбор данных — страницу нужно дополнить, иначе она начнёт врать.
5. **Дата в `privacy.html`** («Обновлено: …») проставлена руками — обновляйте вместе с правками текста.
...
- Версия, дата сборки и ссылки на файлы не вбиты руками: перед выкладкой `.github/stamp-release.py` подставляет в `index.html` значения последнего релиза, поэтому у посетителя кнопки работают без запросов к API. В самом репозитории значения могут отставать — их обновляет деплой.
