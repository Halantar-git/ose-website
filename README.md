...
на обе языковые версии, места подстановки релиза (`data-release-version`, `data-release-date`,
`data-download-os`) на месте в обоих лендингах, и `stamp-release.py` на поддельном релизе подставляет
версию, дату и все три ссылки — по-русски на русской странице и по-английски на английской. Тот же скрипт
стоит в `pages.yml` перед выкладкой: битая разметка не доедет до сайта.
...
- `main.js` и `.github/stamp-release.py` общие для обеих версий: язык берётся из `<html lang="…">`,
  и по нему выбираются формат даты релиза («16 сентября 2026» против «September 16, 2026») и подписи
  лайтбокса.
...
- Сверх этого `main.js` на загрузке спрашивает релиз ещё раз и обновляет версию, дату и адреса — на случай релиза между деплоями; если запрос не прошёл (нет сети, исчерпан лимит 60 запросов в час на IP), остаются значения из разметки. Это единственный внешний запрос со страницы.
- Систему посетителя определяет браузер (`userAgentData`, иначе `userAgent`), и кнопка этой системы получает заливку вместо контура; запрос к API для этого не нужен — адреса проставлены деплоем. iPadOS, Android и ChromeOS считаются неизвестными: файлы релиза на них не запускаются.
