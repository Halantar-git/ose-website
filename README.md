...
- Версия, дата сборки и ссылки на файлы не вбиты руками: `main.js` берёт их из последнего релиза приложения через GitHub API и подставляет версию в шапку, FAQ, блок «Скачать» и JSON-LD. Система определяется по браузеру (`userAgentData`, иначе `userAgent`): главная кнопка ведёт прямо на `setup.exe` / `AppImage` / `dmg` и называется «Скачать для …», а её дубль в нижнем ряду скрывается. В разметке лежат запасные значения и ссылка на страницу релиза — если запрос не прошёл (нет сети, исчерпан лимит неавторизованных запросов к API), всё остаётся как есть. Это единственный внешний запрос со страницы.
