...
assets/img/apple-touch-icon.png  иконка 180×180 для iOS
...
3. **Скриншоты.** Вставлены из приложения: в hero — `assets/img/main.webp`, в галерее `#screens` — остальные 9. Вертикальные скриншоты пульта помечены `shot--tall`, чтобы не растягиваться на всю колонку. Клик по скриншоту открывает его крупно: лайтбокс собран в `assets/js/main.js` без библиотек.
   Alt-тексты написаны по именам файлов — вычитайте их и поправьте под фактические экраны. Особенно `ed-3`, `poll2`, `remote2` и `term`. Снимки одни на обе версии, поэтому править нужно в двух местах: `index.html` и `en/index.html`.
